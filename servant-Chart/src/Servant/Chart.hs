...
import Data.Compact     (Compact, compactWithSharing, getCompact)
...
denv :: DEnv Double
denv = getCompact $ unsafePerformIO denvIO
{-# NOINLINE denv #-}

denvIO :: IO (Compact (DEnv Double))
denvIO = compactWithSharing $ createEnv B.vectorAlignmentFns 1000 700 loadSansSerifFonts
  where
